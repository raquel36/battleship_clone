...
tiradasPosibles= [str(fila)+str(columna) for fila in range(10) for columna in range(10)]
...
todos_barcos={}
#Lista en formato string, necesito esta lista para validar que no
...
def colocar_barco(barcos, screen, cell_size, ANCHO, ALTO, coordenadas_str, barco_actual):
    ...
    for navio, valor in barcos_indice.items():
        ...
        # Funcionalidad del paquete PySimpleGUI que me permite mostrar una pantalla para introducir datos
            layout = [[sg.Text("Seleccione orientacion (h/v) del barco actual "+str(barco_actual)), sg.InputText()],
                    [sg.Text("Seleccione coordenadas, 2 numeros(fila,columna): "), sg.InputText()],
            ...
            orientation = values[0].lower()
            if values[0] is not None or values[1] is not None:

                if values[0].isdigit():
                    if len(values[1])>2:
                        sg.popup("valor introducido incorrecto, debe introducir solo 2 números", keep_on_top=True)
                        return colocar_barco(barcos, screen, cell_size, ANCHO, ALTO, coordenadas_str, barco_actual)
                try:
                    row = int(values[1][0])
                    col = int(values[1][1])
                    # CONTROL DE ERRORES
                    ...
                    if row<0 or row>9 or col<0 or col>9:
                        sg.popup("Números invalidos, debe introducir un rango entre 00 y 99", keep_on_top=True)
                        return colocar_barco(barcos, screen, cell_size, ANCHO, ALTO, coordenadas_str, barco_actual)
                # En caso de que introduzca letras en lugar de números
                    ...
                    sg.popup("Debe introducir 2 números enteros", keep_on_top=True)
                    return colocar_barco(barcos, screen, cell_size, ANCHO, ALTO, coordenadas_str, barco_actual)
            # En caso de que introduzca otra letra que no sea v o h
            ...
            if orientation == "h":
                for i in range(longitud):
                    if (str(row)+str(col+i)) in coordenadas_str:
                        ...
                        return colocar_barco(barcos, screen, cell_size, ANCHO, ALTO, coordenadas_str, barco_actual)
                    # Guardamos las coordenadas en las litas correspondientes
                ...
                for i in range(longitud):
                    if (str(row+i)+str(col)) in coordenadas_str:
                        sg.popup("Este lugar esta ocupado, por favor selecciona otro.", keep_on_top=True)
                        return colocar_barco(barcos, screen, cell_size, ANCHO, ALTO, coordenadas_str, barco_actual)
                ...
                # fila=5, la suma es 10, valido
                    ...
                    if orientacion == 'h':
                        ...
                        for i in range(longitud):
                            ...
                            if (str(fila)+str(columna + i)) in coordenadas_ia_str:
                                posicion_ocupada = True
                                break
                        if posicion_ocupada:
                            continue
                        ...
                        for i in range(longitud):
                            coordenadas_ia_str.append(str(fila)+str(columna + i))
                    elif orientacion == 'v':
                        for i in range(longitud):
                            if (str(fila + i)+ str(columna)) in coordenadas_ia_str:
                                posicion_ocupada = True
                                break
                        if posicion_ocupada:
                            continue
                        for i in range(longitud):
                            coordenadas_ia_str.append(str(fila + i)+str(columna))
        ...
        while continuar:
            ...
            if modo=="buscar":
                ...
                if disparo in coordenadas:
                    ...
                    dibujar_acierto(int(disparo[0]), int(disparo[1]))
                    contadorTiros+=1
                    modo = "hundir"
                ...
                #     coordenadas_user.remove(disparo)
                    ...
                    print(disparo)
                    dibujar_fallo(int(disparo[0]), int(disparo[1]))
                    tiradasPosibles.remove(disparo)
                    print("Agua")
                    if contadorTiros>0:
                        modo = "hundir"
            ...
            # En este modo intentamos hundir el barco
               ...
               # disparo = str(fila)+str(columna)
                ...
                if disparo in coordenadas_str:
                    ...
                    aciertos_computer.append(disparo)
                    coordenadas_str.remove(disparo)
                    ...
                    dibujar_fallo(int(disparo[0]), int(disparo[1]))
                    print("Agua")
                    print("disparo ia",disparo)
                    print("tiradasPosibles", tiradasPosibles)
...
def disparo_user():
    ...
    lista = [  [sg.Text("Seleccione coordenadas, 2 numeros(fila,columna): "), sg.InputText()],
                [sg.Button("Disparar")]]
    ...
    while continuar:
        ...
        if event in (sg.WIN_CLOSED, "Disparar"):
            ...
            try:
                ...
                sg.popup("Números invalidos, debe introducir un rango entre 00 y 99, ha introducido letras", keep_on_top=True)
                return disparo_user()
            except IndexError:
                sg.popup("Debe introducir 2 números enteros", keep_on_top=True)
                return disparo_user()
            except:
                sg.popup("No ha introducido ningún valor", keep_on_top=True)
                lista2 = [  [sg.Text("Quiere salir? q=salir, c=continuar "), sg.InputText()],
                        [sg.Button("ACEPTAR")]]
                vista2 = sg.Window("SALIR?", lista2, location=(ANCHO/3, ALTO - 180), keep_on_top=True, background_color='#03AC13')
                event, values = vista2.read()
                if event in (sg.WIN_CLOSED, "ACEPTAR"):
                    opcion = values[0]
                    if opcion=='q' or opcion=='Q':
                        pygame.quit()
                        sys.exit()
    
                    elif opcion=='c' or opcion=='C' or opcion=='':
                        return disparo_user()
        ...
        if disparo in disparos_realizados_user:
            ...
            if disparo in coordenadas_ia_str:
                dibujar_acierto_user(fila, columna)
                coordenadas_ia_str.remove(disparo)
            else:
...
def turnos():
    ...
    while running:
        if turno == "usuario":
            ...
            disparo_ia(todos_barcos, coordenadas_str, tiradasPosibles)
            if len(coordenadas_str) == 0:
                running = False
...
colocar_barco(barcos, screen, cell_size, ANCHO, ALTO, coordenadas_str, barco_actual)

# Llamamos a la función que nos colocará los barcos del ordenador
colocar_barcos_ia(barco_actual_ia)
print("Coordenadas Ordenador",coordenadas_ia_str)
print(tiradasPosibles)
print("Coordenadas_usuario", coordenadas_str)
turnos()
...
pygame.quit()
#1['14', '15', '16', '17', '18', '64', '74', '84', '94', '66', '67', '68', '52', '62', '38', '39']
